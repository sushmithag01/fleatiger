...
const EditActivityFriendsList = props => {
  const {
    item,
    handleComing,
    isAdded,
  ...
  //
  console.log(selectedComing, 'selectedComingProfile');
  return (
    <TouchableOpacity
    ...
    >
      <View style={styles.logmain}>
        <View style={styles.logmain2}>
          <Image
            source={
              item.petImg == 'https://devapi.fleatiger.com/'
          ...
          <View style={{width: 125}}>
            <Text style={styles.homecardtext1}>{item.petname}</Text>
        ...
        <View style={styles.likesec1}>
          {selectedComing.includes(item.friendId) ? (
            <TouchableOpacity onPress={e => handleComingRemove(item)}>
              <Svg
            ...
            <TouchableOpacity onPress={e => handleComing(item)}>
              <Svg
                xmlns="http://www.w3.org/2000/svg"
                id="icons-RZ_Weiss"
                viewBox="0 0 50 50"
...
export default EditActivityFriendsList;
