...
const FriendsMap = ({ navigation }) => {
  ...
  const extractDataFromApiResponse = apiResponse => {
    ...
    for (const country in countriesData) {
      ...
      if (!markersDictionary[coordinatesKey]) {
        ...
        markersDictionary[coordinatesKey].petName =
  ...
  const handleFindmyFriends = async () => {
    ...
    // console.log("getFriends", getFriends.data)
    if (getFriends.status === 200) {
      setLoading(false);
      setFindMyFrnds(getFriends.data[0]);
  ...
  return (
    <>
      ...
      <SafeAreaView>
        ...
        <View style={styles.space20}></View>
        {findMyFrndsList && findMyFrndsList.length > 0 ? (
                ...
                <View key={index}>
                  <Text
                    style={{
                      color: 'black',
                      fontWeight: 'bold',
                    }}>{`${countryCount.countryName} : ${countryCount.count}`}</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <Text
            ...
            Sorry..! no friends added in your friend list.
          </Text>
        )}
