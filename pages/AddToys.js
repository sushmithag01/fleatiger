...
import ProfileWithFamily from './ProfileWithFamily'
...
import { AddToysHeaderLeft } from '../navigation/CustomBackNavigation';
...
const AddToys = () => {
  ...
  useEffect(()=>{
    navigation.setOptions({
      headerLeft: () => <AddToysHeaderLeft navigation={navigation} data={data}/>,
    });
  },[data])
  // show-name
  useEffect(() => {
  ...
  return (
    <>
      <ScrollView style={[styles.mainpage, styles.padhoz15]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.space20}></View>
        <View>
          <Text style={[styles.createacc, styles.createacc1,{textAlign:"center"}]}>Add Toys</Text>
