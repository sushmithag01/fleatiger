...
const Chat = ({ route, navigation }) => {
  ...
  const [recieverInfo, setRecieverInfo] = useState(route_props || {});
  ...
  const [channel_id, setChannelId] = useState(
    route_props?.friend_list_id ? 'fleatigerFriends' + '_' + route_props?.friend_list_id : ''
  );
                ...
                <View
                  ...
                  <View style={{ width: 250 }}>
                    <Text style={styles.backbtntext} numberOfLines={2}>
                      {recieverInfo.pet_name}
                    </Text>
                    <Text style={styles.secext} numberOfLines={2}>
                      {' '}
                      {recieverInfo.location}
                    </Text>
                  </View>
                </View>
              </View>
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      if (route_props?.friend_list_id) {
        const channel = 'fleatigerFriends' + '_' + route_props.friend_list_id;
          ...
          const messagesFirestore = querySnapshot.docs.map(doc => {
            ...
            return {
              ...
              user: {
                _id: data.user?._id,
                name: data.user?.name,
                avatar: data.user?.image || recieverInfo.pet_image_path
              },
            };
          });
  ...
  async function handleSend(messages) {
    ...
    try {
      ...
      await firestore()
        ...
        .add({
          ...
          user: {
            ...
            image: route_props?.pet_image_path
          },
          sent: true,
          read: false,
          received: recieverInfo.pet_id,
          name: chatType === 1 ? recieverInfo.group_name : recieverInfo.pet_name,
      ...
      if (chatType === 1) {
        ...
        await handleChatInsert(messages);
      }
    } catch (error) {
  ...
  const handleChatInsert = async data => {
    let payload = {
      channel_id: channel_id,
      receiver_id: recieverInfo.pet_id,
      sender_id: senderInfo._id,
      user_id: senderInfo.user_id,
      receiver_friend_list_id: recieverInfo.friend_list_id,
    };

    let chatResponse = await InsertChatApi(payload);
  ...
  const renderActions = props => {
    ...
    console.log("senderInfo && ", senderInfo && 'Hi')
    return (
      <Actions
        containerStyle={{
          position: 'absolute',
