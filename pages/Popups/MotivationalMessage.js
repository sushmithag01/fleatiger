...
const MotivationalMessage = props => {
  const {setCongratsMsg, congratsMsg, MessagePop, status} = props;
  ...
  const handleOkBtn = async()=>{
    setCongratsMsg(false);
    navigation.navigate('SubscriptionStackNavigator',{screen:"Subscription"});
      ...
      <Modal animationType="slide" transparent={true}>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <Text style={styles.modalText}>{MessagePop}</Text>
            {status === 'activity' ? (
              <View
                style={{
                  flexDirection: 'row',
                ...
                <Pressable
                  style={styles.buttonconfirm3}
                  onPress={() => handleOkBtn()}>
                  <Text style={styles.btnText2}>Purchase</Text>
                </Pressable>
                <Pressable
