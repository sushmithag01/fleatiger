...
function CreateGroup({ route }) {
    ...
    useEffect(() => {
        const saveMemberId = async () => {
            ...
            let userInfoId;
            try {
                userInfo = await AsyncStorage.getItem('added_group_members_info');
                userInfoId = await AsyncStorage.getItem('added_group_members_id');
                if (userInfo) {
        ...
        <>
            ...
            <ScrollView style={{ backgroundColor: '#fff' }} showsVerticalScrollIndicator={false}>
                <View style={[styles.fleamain,]}>
                    <Text style={styles.newFrndTitle}>Create Group </Text>
                    ...
                    <View style={{ backgroundColor: '#F1F3F4', margin: 10, flexDirection: 'row', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 5, marginVertical: 10 }}>
                        ...
                        <View style={{ paddingLeft: 10, justifyContent: 'center' }}>
                            <TextInput placeholder='Group Name'
                                ...
                                onChange={(e) => e.nativeEvent.text ? [setGroupName(e.nativeEvent.text), setGroupnameErr('')] :
                                    [setGroupnameErr(ErrorText.GroupNameRequired), setGroupName('')]}
                        ...
                        <View style={{ alignItems: 'center', paddingLeft: 10, padding: 2, }}>
                            <TextInput
                                ...
                                onChange={(e) => e.nativeEvent.text ?
                                    [setGroupDescription(e.nativeEvent.text), setGroupdescErr('')]
                                    :
                                    [setGroupdescErr(ErrorText.GroupDescRequired),setGroupDescription('')]}
                                multiline={true}
                                numberOfLines={4}
                                placeholderTextColor="#888"
                                maxLength={100}
            ...
            <View style={[styles.joinBtn2, { flexDirection: 'row', justifyContent: 'space-between', }]}>
                ...
                <TouchableOpacity onPress={() => { groupName ? setis_addnewMem(true) : setGroupnameErr(ErrorText.GroupNameRequired) }}>
                    <Text style={[styles.homecardtext1, { color: '#ffffff', marginRight: 20, }]}>Next</Text>
                </TouchableOpacity>
            </View>
