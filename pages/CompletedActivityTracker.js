...
import {
  CommonHeaderRight,
  CompletedActivityTrackerHeaderLeft,
  CompletedActivityTrackerHeaderRight,
} from '../navigation/CustomBackNavigation';
import {Alert} from 'react-native';

const CompletedActivityTracker = ({navigation, route}) => {
  const pageName = 'CompletedActivityTracker';
  ...
  const local_data1 = [
    {
      ...
      lable: 'Country 1',
      image: {
        uri: 'https://devapi.fleatiger.com/pets/pet_images/pet_2023-05-24-09-29-48_40.jpg',
      },
    },
    {
      value: '2',
    ...
    },
    {
      ...
      lable: 'Country 3',
      image: {
        uri: 'https://devapi.fleatiger.com/pets/pet_images/pet_2023-05-24-10-40-53_40.jpg',
      },
    },
    {
      value: '4',
  ...
  //  console.log(route.params,"item,dateClicked")
      ...
      navigation.setOptions({
        headerLeft: () => (
          <CompletedActivityTrackerHeaderLeft
            ...
            handleBackPress={() => setModalVisible(!modalVisible)}
          />
        ),
        headerRight: () => (
  ...
  const GetActivityTypeList = async () => {
    ...
    if (Response.status === 200) {
      ...
      // setActivityTypeId(Response?.data[0].activity_type_id)
    } else {
      setActivityType([]);
    }
  };
  ...
  const GetPetFriendListData = async () => {
    ...
    console.log(Response, 'get-frined-afer add');
    if (Response.status === 200) {
      setMyFriendList(Response?.data ?? []);
      setGlobalMyFriendList(Response?.data ?? []);

      // setFriendsIMet(Response?.data)
    } else {
      setMyFriendList([]);
  ...
  const GetCompletedActivityList = async () => {
    ...
    const payload = {
      ...
      // "clicked_activity_id":163,
      //  "date": 2023-8-1,
      status: status,
    };
    const Response = await getCompletedActivityApi(payload);
    // console.log('getCompletedActivityApi', Response, payload);
    if (Response.success == false) {
      ...
      // console.log(Response.data[0].map_coordinates,"Response.data[0].map_coordinates")
      setWayPoints(Response.data[0].map_coordinates);
  ...
  const chooseGallery = async () => {
    ...
    setIsVisible(false);
    console.log(result.assets[0].fileSize, 'result1');
  ...
  const handleSaveImg = async () => {
    ...
    // console.log(ActivityName, ActivityNameErr, ActivityTypeId, ActivityTypeErr, "lee")
    if (
      ActivityTypeErr.length == 0 &&
      ActivityNameErr.length == 0 &&
      ActivityName.length != 0 &&
      ActivityTypeId.length != 0
    ) {
      const UserID = await AsyncStorage.getItem('userId');
      const PetId = await AsyncStorage.getItem('PetId');
      const payload = {
    ...
    <>
      ...
      <View style={styles.mainpage}>
        ...
        <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.homecardinner1}>
            <View style={styles.logmain}>
              <View style={{width: '100%'}}>
                ...
                <View style={[styles.inputContainerAct]}>
                  <TextInput
          ...
          <View style={styles.homecardinner1}>
            <View
              ...
              <Svg
                ...
                <Path
                  className="cls-1"
                  d="M20.64 39.63l2.91-1.6c.9-.49 2-.49 2.9 0l2.91 1.6a3 3 0 003.11-.13l3.09-2.03a5.294 5.294 0 002.39-4.42c0-1.56-.69-3.03-1.88-4.04l-9.12-7.71a3.011 3.011 0 00-3.89 0l-9.12 7.71a5.31 5.31 0 00-1.88 4.04c0 1.78.9 3.44 2.39 4.42l3.09 2.03c.93.61 2.13.66 3.11.13z"
                  fill="#223656"
                />
                <Ellipse
                  className="cls-1"
                ...
                <Path
                  className="cls-1"
                  d="M20.07 18.82c2.15 0 3.9-1.98 3.9-4.41S22.23 10 20.07 10s-3.9 1.98-3.9 4.41 1.74 4.41 3.9 4.41zM29.84 18.82c2.15 0 3.9-1.98 3.9-4.41S32 10 29.84 10s-3.9 1.98-3.9 4.41 1.74 4.41 3.9 4.41z"
                  fill="#223656"
                />
              </Svg>

              <View style={styles.drpdownAct}>
                {ActivityType ? (
                  <View style={styles.inputunitContainer3}>
                    <Dropdown
                      ...
                      // placeholder={ActivityType[0].activity_type_name}
          ...
          {waypoints ? (
            <View style={{padding: 20, height: 350}}>
              <MapPathView waypoints={waypoints} />
            </View>
          ) : null}
        ...
        {goBackPopUp === true ? (
          <>
            <RemoveProfileMessage
              visible={modalVisible}
              onRequestClose={() => {
                setGoBackPopUp(!goBackPopUp);
              }}
              setModalVisible={setGoBackPopUp}
              modalVisible={goBackPopUp}
              handleRemove={() => navigation.replace('AddActivity')}
              text="Are you sure you want to delete this activity?"
              shortText=""
            />
          </>
        ...
        {/* {ShowActivityName == true ? (
          <ActivityNameType
            ...
            setShowActivityName={setShowActivityName}
          />
        ) : (
          ''
        )} */}
...
export default CompletedActivityTracker;
