...
import { getFriendListApi } from './API/ApiCalls';
...
const AddNewChat = props => {
  const { openFriendList, handleRequestNewChat } = props;
  const chatType = 0;
  ...
  const handleClose = () => {
    handleRequestNewChat(!openFriendList);
  };
  ...
  const handleNewChat = data => {
    ...
    // });
    navigation.navigate('Chat', { userInfo: data ,chat_type: chatType});
    handleClose();
  };
  const handleNewChatReq = () => {
    //    navigation.navigate("Messagess",{screen:'UnFriendList'})
    navigation.navigate('MessagesStackNavigator', { screen: 'UnFriendList' });
    handleClose();
  };

  const GetPetnewFriendListData = async () => {
    setLoading(true);
    const UserID = await AsyncStorage.getItem('userId');
    const PetId = await AsyncStorage.getItem('PetId');
    const payload = {
      ...
      <BottomSheet modalProps={{}} isVisible={openFriendList}>
        <View style={[styles.bottomsheetmainAddfriend]}>
                      ...
                      <View style={styles.logmain}>
                        <View style={styles.logmain2}>
                          ...
                          <View style={{ width: 125 }}>
                            ...
                            <Text style={styles.logdate}>{item.country}</Text>
                          </View>
                        </View>
          ...
          <View style={styles.space30}>
            <Text style={styles.findnewbtn}>Could not find?</Text>
          </View>
          <View style={styles.bluebtnsmallmain}>
            <TouchableOpacity
              style={styles.bluebtnsmall}
              onPress={() => handleNewChatReq()}>
