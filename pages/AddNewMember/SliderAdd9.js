...
import {FoodSliderApi} from '../API/ApiCalls';
import AddFoodList from '../AddFoodList';
import {SliderAdd9HeaderLeft} from '../../navigation/CustomBackNavigation';

const SliderAdd9 = ({navigation}) => {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerLeft: () => <SliderAdd9HeaderLeft navigation={navigation} />,
    });
    if (isFocused) {
      getName();
      getSliderData();
  ...
  const getSliderData = async () => {
    const Response = await FoodSliderApi();
  ...
  const handleNext = () => {
    ...
    navigation.navigate('SliderAdd10');
  };
    ...
    <>
      <ScrollView style={[styles.mainpage, styles.padhoz15]}>
        ...
        {data?.length == 0 ? (
          ...
          <>
            {data.map((item, key) => {
              return <AddFoodList item={item} data={data} setData={setData} />;
            })}
          </>
        )}
